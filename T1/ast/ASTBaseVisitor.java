...
public abstract class ASTBaseVisitor<T> {
	...
	protected T visit(AST node) {
		switch(node.kind) {
			...
			case FUNCDEC_NODE:  return visitFuncDecl(node);
			case MT_NODE:  return visitMt(node);
			case NOTEQ_NODE:  return visitNotEq(node);
			case LEQT_NODE:  return visitLEq(node);
			case MEQT_NODE:  return visitMEqt(node);
			case PARAMLIST_NODE:  return visitParamList(node);
	...
	protected abstract T visitMt(AST node);

	protected abstract T visitNotEq(AST node);

	protected abstract T visitLEq(AST node);

	protected abstract T visitMEqt(AST node);
